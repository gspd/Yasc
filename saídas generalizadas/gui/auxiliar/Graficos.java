...
public class Graficos {
    ...
    public void criarProcessamentoTempoUser(List<Tarefa> tarefas, RedeDeFilas rdf) {
        ...
        JFreeChart user2 = ChartFactory.createXYLineChart(
                "Use of total resources  through time"
                + "\nUsers", //Titulo
                "Time (seconds)", // Eixo X
                "Rate of total use of resources  (%)", //Eixo Y
                dadosGrafico, // Dados para o grafico
        ...
        if (job != null) {
            ...
            if (job.getEstado() != Tarefa.CANCELADO) {
                ...
                JFreeChart jfc = ChartFactory.createStackedBarChart(
                        "Units usage for task " + idTarefa, //Titulo
                        "", // Eixo X
    ...
    public ChartPanel criarGraficoAproveitamento(List<Tarefa> tarefas) {
        ...
        dadosMflopProcessados.addValue((tamanhoTotal / (mflopDesperdicado + tamanhoTotal)) * 100.0, "Usefull Processing", "Units Usage");
        dadosMflopProcessados.addValue((mflopDesperdicado / (mflopDesperdicado + tamanhoTotal)) * 100.0, "Wasted Processing", "Units Usage");
