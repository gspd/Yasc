...
public class SalvarResultadosHTML {
    ...
    public void setSatisfacao(Object[][] satisfacao) {
        if (satisfacao.length > 1) {
            this.satisfacao = "<ul>";
            for (Object[] satisfacao1 : satisfacao) {
                this.satisfacao += "<li><strong>User" + satisfacao1[0] + "</strong> = " + satisfacao1[1] + " %</li>\n";
            }
            this.satisfacao += "</ul>";
        }
    }
    ...
    public void gerarHTML(File diretorio) throws IOException {
        ...
        if (!arquivo.exists()) {
            ImageIO.write(ImageIO.read(yasc.gui.JPrincipal.class.getResource("imagens/fundo_html.jpg")), "jpg", arquivo);
        }
        arquivo = new File(diretorio, "Logo_iSPD_128.png");
        if (!arquivo.exists()) {
            ImageIO.write(ImageIO.read(yasc.gui.JPrincipal.class.getResource("imagens/Logo_iSPD_128.png")), "png", arquivo);
        }
        arquivo = new File(diretorio, "Logo_UNESP.png");
        if (!arquivo.exists()) {
            ImageIO.write(ImageIO.read(yasc.gui.JPrincipal.class.getResource("imagens/Logo_UNESP.png")), "png", arquivo);
        }
    }
}